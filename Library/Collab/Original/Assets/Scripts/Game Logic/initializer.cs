...
public class initializer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            int numPerRing = 9;
            for (int x = 0; x < numPerRing; x++)
            {
                float dist = 18;
                Vector3 dir = chompy.FastestPathOnSphere(transform.right, (transform.position - platplanet.transform.position).normalized);
                var p = Instantiate(platform, transform.position, Quaternion.identity);
                UnityEngine.Debug.Log("dgsdg " + x * 360 / numPerRing);
                p.GetComponent<platforms>().Setup(dir, dist, x * 360 / numPerRing, 0, 0);
            }
        }
